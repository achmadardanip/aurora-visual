...
| Screening asal media (Tahap 1) | implemented, live_verified terbatas | Memeriksa byte asli, nama/presence metadata dan marker C2PA/JUMBF; output hanya di `extensions.aurora_visual.screening`. Saat provider Hive dipilih, deteksi AI-generated/deepfake model `hive/ai-generated-and-deepfake-content-detection` (V3, byte asli, ambang provider 0.9, atribusi generator) diverifikasi live 13 September 2026 pada foto nyata. Bukan validator C2PA atau detektor watermark tervalidasi, dan tidak mengubah S/C/U atau kebenaran caption. |
...
| `make test` | Ruff, format, TypeScript, Prettier dan golden JCS lulus; **182 passed**, 0 failed, 3 warning yang telah diketahui (termasuk 47 test Hive mock httpx, 9 test kebijakan Hive V3-wajib/V2-opsional, 5 test deteksi V3 termasuk respons asli >100 kelas, dan conftest hermetik terhadap `.env` developer). | [pytest.xml](artifacts/reports/pytest.xml); `checks.log` adalah snapshot run sebelumnya. |
...
Pada pemeriksaan akhir, API lokal memberikan `/health` **200 ok** dan `/ready` **200 degraded**: database, worker, demo, warna lokal, OCR, **OpenCLIP** (`ViT-B-32-quickgelu/openai`, bobot terunduh), **Ollama** (`qwen2.5:7b-instruct` lokal), **Hive V3 VLM** dan **Hive V3 deteksi AI/deepfake** siap (`ok`); kapabilitas Hive V2 melaporkan status `optional` (tidak menurunkan readiness; model preview V2 memerlukan project key enterprise). Status `degraded` kini hanya berasal dari head penelitian (`AURORA_CHECKPOINT`) yang memang belum tersedia. Instansi lokal siap dipakai langsung: parser aturan/Ollama/Hive V3, backbone lokal/OpenCLIP, serta deteksi AI/deepfake Hive V3 dapat dipilih di UI tanpa konfigurasi tambahan.
