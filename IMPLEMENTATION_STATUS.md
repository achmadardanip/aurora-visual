...
| Integrasi Hive tahap 0–3 | implemented; mock_verified; live belum diverifikasi | Tahap 1 V2 kirim byte asli (deteksi AI/deepfake + metadata provider), Tahap 2 V3 VLM atomizer dengan validasi caption kanonis yang sama seperti Ollama, Tahap 3 V2 preview ternormalisasi (OCR/objek/scene/orang/logo/selebriti) plus observasi VLM V3 yang hanya dapat mengangkat Unobservable → Supported/Contradicted atas bukti eksplisit berregion; ketidakcocokan dengan jalur lokal menahan hasil ke Unobservable. Kredensial server-side; opt-in per analisis mode live; mode demo dipaksa lokal. Terjemahan bayangan opsional tidak mengubah caption kanonis. Uji unit/integrasi memakai mock httpx; kompatibilitas live Hive belum diverifikasi tanpa project key nyata. |
...
| `make test` | Ruff, format, TypeScript, Prettier dan golden JCS lulus; **159 passed**, 0 failed, 3 warning yang telah diketahui (termasuk 47 test Hive mock httpx). | [pytest.xml](artifacts/reports/pytest.xml); [checks.log](artifacts/reports/checks.log) adalah snapshot run sebelumnya. |
...
| `AURORA_HIVE_ENABLED`, `AURORA_HIVE_V3_SECRET_KEY`, `AURORA_HIVE_V2_*_API_KEY` | Aktifkan pemrosesan eksternal Hive (mode live saja). V3 secret wajib untuk atomizer dan observasi multimodal; project key V2 per kapabilitas (origin/OCR/objek/scene/orang/logo/selebriti/terjemahan) atau shared key sebagai fallback. Semua kredensial server-side; pengguna memilih Hive per analisis dengan disclosure egress di UI. |
