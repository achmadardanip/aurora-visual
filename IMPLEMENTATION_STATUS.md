...
| Integrasi Hive tahap 0–3 | implemented; V3 live_verified terbatas; V2 mock_verified | **Kebijakan V3-wajib/V2-opsional:** secret V3 bersifat wajib dan divalidasi pada konfigurasi (env/overlay/UI) serta dijaga di pipeline (`HIVE_V3_REQUIRED`); project key V2 opsional — tanpa kunci V2, Tahap 1 sepenuhnya lokal, model preview Tahap 3 dilewati (status `optional`, tanpa peringatan/partial), dan egress byte asli hanya terjadi bila origin key V2 dikonfigurasi. Tahap 2 V3 VLM atomizer dengan validasi caption kanonis yang sama seperti Ollama, Tahap 3 observasi VLM V3 yang hanya dapat mengangkat Unobservable → Supported/Contradicted atas bukti eksplisit berregion; ketidakcocokan dengan jalur lokal menahan hasil ke Unobservable. Jalur V3 diverifikasi live 13 September 2026 (lihat tabel bukti); jalur V2 tetap mock httpx. Terjemahan bayangan V2 opsional tidak mengubah caption kanonis. Kredensial server-side; opt-in per analisis mode live; mode demo dipaksa lokal. Peringatan review (`HIVE_ATOMIZER_REVIEW`, `LLM_REVIEW`) tidak lagi menandai run partial. |
...
| `make test` | Ruff, format, TypeScript, Prettier dan golden JCS lulus; **177 passed**, 0 failed, 3 warning yang telah diketahui (termasuk 47 test Hive mock httpx, 9 test kebijakan Hive V3-wajib/V2-opsional + parser Ollama, dan conftest yang kini hermetik terhadap `.env` developer). | [pytest.xml](artifacts/reports/pytest.xml); `checks.log` adalah snapshot run sebelumnya. |
...
| Analisis live Hive **V3-only** (tanpa project key V2) | Lulus pada 13 September 2026 via `POST /api/v1/analyze` mode live provider `hive`: atomizer V3 VLM `status=ok` (1 atom), observasi VLM V3 `status=ok` (merah solid → Supported berregion, confidence ~1.0), `egress.original_media_stage1=false`, stage 1 lokal, run `completed` tanpa peringatan HIVE_ gagal. Jalur V2 tetap mock-only. | Output terminal `e2e_verify`; bundle tersimpan di riwayat kasus lokal. |
...
Pada pemeriksaan akhir, API lokal memberikan `/health` **200 ok** dan `/ready` **200 degraded**: database, worker, demo, warna lokal, OCR, **OpenCLIP** (`ViT-B-32-quickgelu/openai`, bobot terunduh), **Ollama** (`qwen2.5:7b-instruct` lokal), dan **Hive V3 VLM** siap (`ok`); kapabilitas Hive V2 melaporkan status `optional` (tidak menurunkan readiness). Status `degraded` kini hanya berasal dari head penelitian (`AURORA_CHECKPOINT`) yang memang belum tersedia. Instansi lokal siap dipakai langsung: parser aturan/Ollama/Hive V3 serta backbone lokal/OpenCLIP dapat dipilih di UI tanpa konfigurasi tambahan.
