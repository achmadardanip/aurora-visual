...
Hasil analysis dapat memuat `extensions.aurora_visual.screening`. Objek lokal ini berisi target media, field metadata yang hadir, klasifikasi marker perangkat lunak, hasil validasi manifest C2PA (`status` `verified`/`present_unverified`/`not_detected`/`unavailable`, kode `validation_status`, claim generator, signer, digital source types), slot watermark, status detector, dan keterbatasan. Ia tidak mengubah kontrak top-level, `analysis.visual_assessments`, `retrieval`, atau `decision`. `verified` berarti tanda tangan manifest lolos validasi trust store c2pa-python — bukan audit rantai kepercayaan penuh; `present_unverified` berarti manifest ada tetapi tidak dipercaya. Slot `provenance.watermark` diisi provider `synthid-detector` hanya saat opsi `extensions.aurora_visual.options.synthid_detector` dipilih pada mode live dan gateway dikonfigurasi; jejaknya ada di `extensions.aurora_visual.synthid`. `unavailable` pada detector berarti tidak dinilai, bukan hasil negatif.
