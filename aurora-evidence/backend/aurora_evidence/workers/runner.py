"""Persistent job worker for module 2 (mirrors the proven module-1 design)."""
...
from aurora_evidence.core.retrieval import retrieve
from aurora_evidence.models.db import Audit, Base, Case, Job, WorkerState, database
from aurora_evidence.services.cases import snapshot
...
def execute(settings, session, job_id):
    ...
    try:
        ...
        with session.begin() as s:
            ...
            current_job.status = "partial" if output.retrieval.run.status == "partial" else "succeeded"
            current_job.result, current_job.progress = result, "Pencarian bukti selesai"
            case.bundle, case.updated_at = result, time.time()
            snapshot(s, output, "retrieval", job.run_id)
            s.add(
                Audit(
                    id=str(uuid4()),
                    case_id=job.case_id,
                    ...
                    details={
                        "run_id": job.run_id,
                        "evidence_count": len(output.retrieval.evidence_list),
                        "signal_count": len(output.retrieval.forensic_signals),
        ...
        fail(
            ...
            job_id,
            "RETRIEVAL_FAILED",
            "Pencarian bukti gagal. Periksa konfigurasi provider atau input; riwayat tetap tersimpan.",
            False,
            detail,
        )
...
class Worker:
    ...
    def claim(self):
        with self.session.begin() as s:
            ...
            if not job:
                return None
            job.status, job.progress = "running", "Memulai pencarian bukti"
            job.attempts += 1
    ...
    def run_once(self, isolated=True):
        ...
        env = {
            **os.environ,
            ...
            "AURORA_EVIDENCE_DATA_DIR": str(self.settings.data_dir.resolve()),
        }
        process = subprocess.Popen(
            [sys.executable, "-m", "aurora_evidence.workers.runner", "--execute", job_id], env=env
        )
...
if __name__ == "__main__":
    settings = Settings()
    settings.prepare()
    from aurora_evidence.services.settings_store import SettingsStore
